...
- `qwen3:14b-q8_0` ダウンロード済み。人格焼き込み済みモデル `jarvis-local` 生成済み
...
| 役割 | モデル | 備考 |
|---|---|---|
| 軽量級(常用) | Qwen3 14B (`qwen3:14b-q8_0`, 約 16GB) | 8bit 量子化で精度優先。`num_ctx 16384`・思考モードは `/think` で選択的に使用 |
| 軽量級(16GB 機) | Qwen3 14B (`qwen3:14b`, 約 9GB) ほか | `build.sh --profile lite / lite-alt`。RAM から自動選択も可(README の機体別プロファイル参照) |
